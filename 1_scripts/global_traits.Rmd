...
try_data <- rtry_import("../../1_data/TRY_data/42059.txt") #%>% 
...
georef <- try_data %>% rtry_select_anc(59, 60) # keep lon/lat
...
```{r}
global_traits_w %>% 
  filter(OriginalName == "Empetrum nigrum") %>% 
  ggplot(aes(y = `Leaf phosphorus (P) content per leaf dry mass`)) + 
  geom_boxplot()
