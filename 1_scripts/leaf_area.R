...
#list.of.files <- dir(path = paste0("2_input/leaf_scans/analysis_ready/"), pattern = "jpeg|jpg", recursive = TRUE, full.names = TRUE)
#new.folder <- "3_output/x_temp/"
#output.folder <- "3_output/1_data_products/"
#LeafArea.raw_total <- plyr::ldply(list.of.files, loop.files)

# test run.ij
dd <- run.ij(set.directory = "2_input\\leaf_scans\\analysis_ready\\", distance.pixel = 237, known.distance = 2, log = TRUE, low.size = 0.1, trim.pixel = 58, trim.pixel.right = 150, trim.pixel.top = 1500, save.image = TRUE)
dd$summary

ex.dir <- eximg()
run.ij(set.directory = "2_input/leaf_scans/analysis_ready")
